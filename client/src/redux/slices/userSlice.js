...
const handleTokenResponse = (response) => {
    ...
    const authTokenTerm = 300; // minutes
    const refreshTokenTerm = 30; // days

    const authTokenExpirationDate = currentTimestamp + ((authTokenTerm - 1) * 60 * 1000);
    const refreshTokenExpirationDate = currentTimestamp + (refreshTokenTerm * 24 * (60 - 1) * 60 * 1000);

    localStorage.setItem("authToken", response.accessToken);
    localStorage.setItem("refreshToken", response.refreshToken);
    localStorage.setItem("authTokenExpirationDate", authTokenExpirationDate.toString());
    localStorage.setItem("refreshTokenExpirationDate", refreshTokenExpirationDate.toString());
};
...
export const {setUserData, setToken, clearUserData, setTokenExpirationDate} = userSlice.actions;
