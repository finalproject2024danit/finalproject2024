...
const moveCells = (initCells, direction) => {
  if (!Array.isArray(initCells)) {
    console.error('Invalid initCells:', initCells);
    return []; // або поверніть пустий масив
  }

  const cells = cloneDeep(initCells);
  const matrix = Array.from(new Array(4), () => Array(4).fill(0));
